...
- [x] 3.4 Persist models to `models/` via joblib
...
- [x] 4.3 Test: optimizer never worse than the 0.5 default on a toy set
- [x] 4.4 `evaluate.py` writes `reports/evaluation.json` (single source of truth)
...
- [x] 7.1 `model-card` skill: generate `reports/model_card.md` from `evaluation.json`
- [x] 7.2 Executive summary: dollars saved vs. naive rule at the alert budget, with charts

## 8. Governance & polish (R10)
- [x] 8.1 Raise test coverage on `src/` past 80%
- [x] 8.2 Ensure ruff is clean; CI green on the committed sample
- [x] 8.3 Fill results into README tables from `evaluation.json`
- [x] 8.4 Final pass: confirm no data/secrets committed; seeds fixed; runs are reproducible
